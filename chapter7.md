...
1. **学习曲线**：团队需要多长时间才能高效使用？
2. **可扩展性**：当项目规模增长10倍时，工具是否仍然适用？
3. **生态系统**：是否有活跃的社区、插件、教程？
4. **跨平台能力**：能否轻松部署到Web、移动端、桌面？
5. **维护成本**：长期维护和更新的工作量如何？
...
:: 购买确认
<<if $inventory.includes("宝剑")>>
    你获得了宝剑！剩余金币：$gold
    
    <<if $karma > 5>>
        店主微笑着说："看得出你是个好人，这把剑会保护你的。"
    <<else>>
        店主面无表情地完成了交易。
    <</if>>
<</if>>
